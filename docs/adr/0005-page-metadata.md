...
**소셜 카드 이미지는 범위에서 뺐다:** 기본 카드 이미지를 새로 디자인해야 하고, 글에는 대표 이미지 개념이 아직 없다. `twitter:card`를 `summary`로 둔 것도 그래서다 — `summary_large_image`는 이미지가 있어야 의미가 있다. 되돌릴 단위가 다르므로 분리했다. → [#9](https://github.com/flameware/kirby-blog/issues/9)
...
- 소셜 카드에는 아직 이미지가 없다. `summary` 카드는 텍스트만 보여준다. → [#9](https://github.com/flameware/kirby-blog/issues/9)
